import React from 'react'

export const NFTqs3 = () => {
    return (
        <div className='w-full h-fit'>
            <div className='grid justify-start items-center gap-6'>
                <h1 className='text-4xl font-semibold tracking-wider'>3. How NFT works?</h1>
                <p className='text-2xl tracking-wider text-justify px-12 font-raleway font-thin'>
                    NFTs work by leveraging blockchain technology to create unique, verifiable digital ownership.
                    Each NFT contains a distinct code that distinguishes it from others, making it non-fungible.
                    When a creator mints an NFT, the blockchain records the creation and establishes its ownership.
                    Smart contracts govern transactions, ensuring transparent, secure transfers.
                    These NFTs can represent various digital or physical assets, such as art, music, or virtual items.
                    They are bought, sold, and traded on NFT marketplaces, with transactions publicly recorded on the blockchain.
                    This immutable ledger preserves ownership history, proving the authenticity and rarity of the item, granting value to
                    NFTs in today's digital economy.
                </p>
            </div>
        </div>
    )
}
